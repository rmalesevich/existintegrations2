<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Manage Toggl Track Connection for {{ Auth::user()->name }}
        </h2>
    </x-slot>
                ...
                @endif
                    ...
                    <div class="mb-8">
                        <h3 class="font-semibold text-xl text-gray-800 leading-tight mb-4">
                            {{ __('app.connectedAs', ['username' => $user->togglUser->external_user_id]) }}
                        </h3>
                    </div>
                    ...
                    <div class="mb-12">
                        <div class="text-sm text-gray-900">
                            ...
                            @csrf
                                            ...
                                            <select id="project-{{ $project->project_id }}" name="project[{{ $project->project_id }}]" class="form-select appearance-none block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding bg-no-repeat border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none">
                                                ...
                                                @foreach ($attributes as $attribute)
                                                    <option value="{{ $attribute['attribute'] }}"
                                                        @if ($project->attribute == $attribute['attribute'])
                                                            selected="selected"
                                                        @endif
                                ...
                                <button 
                                    type="submit" 
                                    title="{{ __('app.attributeButtonTitle', ['service' => 'Toggl Track']) }}"
                                    class="inline-block px-6 py-2.5 bg-green-500 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-green-600 hover:shadow-lg focus:bg-green-600 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-green-700 active:shadow-lg transition duration-150 ease-in-out">
                                    {{ __('app.attributeButton', ['service' => 'Toggl Track']) }}
                                </button>
                            </form>
                        </div>
                    </div>
                    ...
                    <div class="mb-4 pb-4">
                        <div class="float-right">
                            <form action="{{ route('toggl.refreshProjects') }}" method="post">
                                @csrf
                                <button 
                                    type="submit" 
                                    ...
                                    {{ __('app.refreshCategory', ['service' => 'Toggl Track', 'category' => 'Projects']) }}
                                </button>
                            </form>
                        </div>
                    </div>
                    ...
                    <div class="mb-12">
                        <div class="text-sm text-gray-900">
                            <p class="mb-4">
                                {{ __('app.zeroOutText', ['service' => 'Toggl Track']) }}
                            </p>
                            <form action="{{ route('toggl.zero') }}" method="post">
                                @csrf
                                @php
                                ...
                                <button 
                                    ...
                                    {{ __('app.zeroOutButton', ['service' => 'Toggl Track']) }}
                                </button>
                            </form>
                        </div>
                    </div>
                    ...
                    <div class="mb-4 pb-4">
                        <div class="float-right">
                            <form action="{{ route('toggl.disconnect') }}" method="post">
                                @method('DELETE')
                                @csrf
                                @php
